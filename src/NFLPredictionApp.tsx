...
const NFLPredictionApp = () => {
  ...
  useEffect(() => {
    Promise.all([loadSeasonData(FIT_SEASON), loadSeasonData(TEST_SEASON)])
      .then(([fit, test]) => {
        setNflData(fit);
        setTestData(test);
        setSelectedGame(fit.upcoming_games[0] ?? null);
        setLoading(false);
      })
      .catch((err) => {
  ...
  const calculateTeamScore = (teamAbbr) => {
    ...
    return scoreTeam(nflData.teams[teamAbbr], weights);
  };
